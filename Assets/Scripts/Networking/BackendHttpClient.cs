...
public class BackendHttpClient : MonoBehaviour
{
    ...
    public string baseUrl = "http://127.0.0.1:8000"; // local dev default
    [Tooltip("Optional: set a different base URL when running on Android (e.g., http://<your-mac-ip>:8000)")]
    public string androidOverrideBaseUrl = "";
    ...
    private void Awake()
    {
        ...
        // Use your host machine IP if the Python server runs on your Mac.
        if (!string.IsNullOrEmpty(androidOverrideBaseUrl))
        {
            baseUrl = androidOverrideBaseUrl;
        }
#endif
    }
